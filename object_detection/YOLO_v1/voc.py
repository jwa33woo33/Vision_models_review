...
class VOCDataset(Dataset):
    ...
    def __getitem__(self, idx):
        path = self.paths[idx]
        # img = cv2.imread(path)
        img = Image.open(path) # Fixed(youngwoon)
        img = img.convert('RGB') # Fixed(youngwoon)
        img = np.asarray(img) # Fixed(youngwoon)
...
def test():
    ...
    label_txt = ['data/voc2012.txt'] # ['data/voc2007.txt', 'data/voc2012.txt']
