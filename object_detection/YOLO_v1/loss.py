...
class Loss(nn.Module):
    ...
    def compute_iou(self, bbox1, bbox2):
        ...
        Args:
            bbox1: (Tensor) bounding boxes, sized [N, 4].
            bbox2: (Tensor) bounding boxes, sized [M, 4].
        Returns:
        ...
        lt = torch.max(
            ...
            bbox2[:, :2].unsqueeze(0).expand(N, M, 2) # [M, 2] -> [1, 1, 2] -> [N, M, 2]
        )
    ...
    def forward(self, pred_tensor, target_tensor):
        ...
        coord_pred = pred_tensor[coord_mask].view(-1, N)             # pred tensor on the cells which contain object. [n_coord, N]
        ...
        for i in range(0, bbox_target.size(0), B):
            ...
            # target bbox at i-th cell. Because target boxes contained by each cell are identical in 
            # current implementation, enough to extract the first one.
            ...
            # "We want the confidence score to equal the intersection over union(IoU) between 
            # the prediction box and the ground truth" from the YOLO paper
            bbox_target_iou[i+max_index, torch.LongTensor([4]).to(device)] = (max_iou).data.cuda()
