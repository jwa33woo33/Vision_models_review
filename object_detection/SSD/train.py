from data import *
from utils.augmentations import ConvertColor, SSDAugmentation
...
parser.add_argument('--dataset', default='COCO', choices=['VOC', 'COCO'], type=str, help='VOC or COCO')
...
def train():
    if args.dataset == 'COCO':
        if args.dataset_root == VOC_ROOT:
            if not os.path.exists(COCO_ROOT):
                parser.error('Must specify dataset_root')
            print("WARNING: usng default coco dataset_root")
    ...
    if args.resume:
        ...
        ssd_net.load_weight(args.resume)
    else: 
    ...
    if iteration != 0 and iteration %5000 == 0:
        ...
        torch.save(ssd_net.state_dic(), args.save_folder+''+args.dataset+'.pth')

def adjust_learning_rate(optimizer, gamma, step):
    lr = arg.lr * (gamma**(step))
