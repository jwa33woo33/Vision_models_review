...
class Compose(object):
    ...
    def __call__(self, img, boxes =None, labels = None):
        for t in self.transforms:
            img, box, labels = t(img, boxes, labels)
...
class RandomSaturation(object):
    def __init__(self, lower=0.5, upper=1.5):
        ...
        assert self.lower >=0, "contrast lower must be >= 0"
    ...
    def __call__(self, image, boxes=None, labels=None):
        ...
        while True:
            ...
            for _ in range(50):
                ...
                m2 = (rect[2] < centers[:, 0])*(rect[2]< centers[:,1])
                mask = m1*m2
                ...
                current_boxes[:, :2] = np.max(current_boxes[:,:2], rect[:2])
                current_boxes[:, :2] -= rect[:2]
                current_boxes[:, 2:] = np.min(current_boxes[:,2:], rect[2:])
                current_boxes[:, 2:] -= rect[:2]
